...
public class Base64 {
    ...
    public final int encodeIntoByteArrayImpl$kotlin_stdlib(byte[] source, byte[] destination, int destinationOffset, int startIndex, int endIndex) {
        ...
        switch (endIndex - sourceIndex) {
            ...
            case 2:
                ...
                int byte7 = source[sourceIndex5] & 255;
                int bits3 = (byte6 << 10) | (byte7 << 2);
    ...
    private final int decodeImpl(byte[] source, byte[] destination, int destinationOffset, int startIndex, int endIndex) {
        ...
        int destinationIndex = destinationOffset;
        while (symbol1 < endIndex) {
            if (byteStart == -8 && symbol1 + 3 < endIndex) {
                int sourceIndex = symbol1 + 1;
                int symbol2 = decodeMap[source[symbol1] & 255];
                int sourceIndex2 = sourceIndex + 1;
                int symbol3 = decodeMap[source[sourceIndex] & 255];
                int sourceIndex3 = sourceIndex2 + 1;
                int symbol4 = decodeMap[source[sourceIndex2] & 255];
                int sourceIndex4 = sourceIndex3 + 1;
                int symbol5 = decodeMap[source[sourceIndex3] & 255];
        ...
        if (sourceIndex5 < endIndex) {
            ...
            StringBuilder sbAppend2 = new StringBuilder().append("Symbol '").append((char) symbol6).append("'(");
            String string2 = Integer.toString(symbol6, CharsKt.checkRadix(8));
            Intrinsics.checkNotNullExpressionValue(string2, "toString(this, checkRadix(radix))");
            throw new IllegalArgumentException(sbAppend2.append(string2).append(") at index ").append(sourceIndex5 - 1).append(" is prohibited after the pad character").toString());
        }
        return destinationIndex - destinationOffset;
    }

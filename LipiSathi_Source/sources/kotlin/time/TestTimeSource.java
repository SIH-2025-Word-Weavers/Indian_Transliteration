...
public final class TestTimeSource extends AbstractLongTimeSource {
    ...
    public final void m1661plusAssignLRDsOJo(long duration) {
        long longDelta = Duration.m1571toLongimpl(duration, getUnit());
        ...
        if ($i$f$isSaturated == 0) {
            ...
            if ((j ^ longDelta) >= 0 && (j ^ newReading) < 0) {
                m1660overflowLRDsOJo(duration);
            }
            this.reading = newReading;
            return;
        }
        long half = Duration.m1528divUwyO8pc(duration, 2);
        long $this$isSaturated$iv = Duration.m1571toLongimpl(half, getUnit());
        if (!((($this$isSaturated$iv - 1) | 1) == Long.MAX_VALUE)) {
            long readingBefore = this.reading;
            try {
                m1661plusAssignLRDsOJo(half);
                m1661plusAssignLRDsOJo(Duration.m1560minusLRDsOJo(duration, half));
                return;
    ...
    private final void m1660overflowLRDsOJo(long duration) {
        throw new IllegalStateException("TestTimeSource will overflow if its reading " + this.reading + DurationUnitKt.shortName(getUnit()) + " is advanced by " + ((Object) Duration.m1574toStringimpl(duration)) + '.');
    }
}

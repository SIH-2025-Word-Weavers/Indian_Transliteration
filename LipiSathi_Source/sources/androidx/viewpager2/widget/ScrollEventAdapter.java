...
final class ScrollEventAdapter extends RecyclerView.OnScrollListener {
    ...
    public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
        ...
        if (this.mDispatchSelected) {
            ...
            if (dy <= 0) {
                if (dy == 0) {
                    scrollingForward = (dx < 0) == this.mViewPager.isRtl();
                }
            }
            int i = (!scrollingForward || this.mScrollValues.mOffsetPx == 0) ? this.mScrollValues.mPosition : this.mScrollValues.mPosition + 1;
            this.mTarget = i;

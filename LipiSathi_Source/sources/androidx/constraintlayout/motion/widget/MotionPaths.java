...
class MotionPaths implements Comparable<MotionPaths> {
    ...
    void initPath(KeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        float f2 = startTimePoint.width;
        float startCenterX = (f2 / 2.0f) + f;
        float f3 = startTimePoint.y;
        float position2 = startTimePoint.height;
        float startCenterY = f3 + (position2 / 2.0f);
        ...
        this.width = (int) (f2 + (scaleX * scaleWidth));
        this.height = (int) (position2 + (scaleY * scaleHeight));
        ...
        while (true) {
            ...
            if (i2 < dArr.length) {
                ...
                if (Float.isNaN(path_rotate)) {
                    ...
                    view.setRotation((float) (((double) rot) + ((double) path_rotate) + Math.toDegrees(Math.atan2(dy, dx))));
                }
                int l = (int) (v_x + 0.5f);
                int t = (int) (v_y2 + 0.5f);

    ...
    private static class Arc {
        ...
        Arc(int mode, double t1, double t2, double x1, double y1, double x2, double y2) {
            ...
            if (this.linear || Math.abs(dx) < EPSILON || Math.abs(dy) < EPSILON) {
                ...
                this.mY1 = y1;
                this.mY2 = y2;
                double dHypot = Math.hypot(dy, dx);
                this.mArcDistance = dHypot;
                this.mArcVelocity = dHypot * this.mOneOverDeltaTime;
                double d = this.mTime2;
                double d2 = this.mTime1;
                this.mEllipseCenterX = dx / (d - d2);
                this.mEllipseCenterY = dy / (d - d2);
                return;
            }
        ...
        private void buildTable(double x1, double y1, double x2, double y2) {
            ...
            while (true) {
                ...
                double dist2 = dist;
                double dist3 = i;
                double angle = Math.toRadians((dist3 * 90.0d) / ((double) (dArr.length - 1)));
                ...
                if (i > 0) {
                    double a2 = px - lx;
                    double b2 = py - ly;
                    double dist4 = Math.hypot(a2, b2) + dist2;
                    ourPercent[i] = dist4;
                    dist2 = dist4;
                }
                lx = px;
            ...
            while (true) {
                ...
                if (i3 < dArr3.length) {
                    ...
                    if (index >= 0) {
                        ...
                        double d = dArr4[p1];
                        double ans = (((double) p1) + ((pos - d) / (dArr4[p2] - d))) / ((double) (dArr4.length - 1));
                        this.mLut[i3] = ans;
                    }
                    i3++;
                } else {

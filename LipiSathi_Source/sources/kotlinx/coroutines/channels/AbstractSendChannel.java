...
public abstract class AbstractSendChannel<E> implements SendChannel<E> {
    ...
    public final Object mo1707trySendJP2dKIU(E element) {
        Object result = offerInternal(element);
        if (result == AbstractChannelKt.OFFER_SUCCESS) {
            return ChannelResult.INSTANCE.m1727successJP2dKIU(Unit.INSTANCE);
        }
        if (result == AbstractChannelKt.OFFER_FAILED) {
            Closed<?> closedForSend = getClosedForSend();
            return closedForSend == null ? ChannelResult.INSTANCE.m1726failurePtdJZtk() : ChannelResult.INSTANCE.m1725closedJP2dKIU(helpCloseAndGetSendException(closedForSend));
        }
        if (result instanceof Closed) {
            return ChannelResult.INSTANCE.m1725closedJP2dKIU(helpCloseAndGetSendException((Closed) result));
        }
    ...
    /* JADX WARN: Code duplicated, block: B:29:0x008f A[EDGE_INSN: B:29:0x008f->B:30:0x0099 BREAK  A[LOOP:0: B:3:0x000f->B:45:?]] */
    ...
    /* JADX WARN: Code duplicated, block: B:45:? A[LOOP:0: B:3:0x000f->B:45:?, LOOP_END, SYNTHETIC] */
        ...
        while (true) {
            if (isFullImpl()) {
                ...
                if (offerResult == AbstractChannelKt.OFFER_SUCCESS) {
                    ...
                    cont.resumeWith(Result.m200constructorimpl(Unit.INSTANCE));
                    break;
                }
    ...
    public final void helpCloseAndResumeWithSendException(Continuation<?> continuation, E e, Closed<?> closed) {
        ...
        if (function1 != null && (it = OnUndeliveredElementKt.callUndeliveredElementCatchingException$default(function1, e, null, 2, null)) != null) {
            ...
            Result.Companion companion = Result.INSTANCE;
            continuation.resumeWith(Result.m200constructorimpl(ResultKt.createFailure(it)));
        } else {
            Result.Companion companion2 = Result.INSTANCE;
    ...
    public boolean cancel(Throwable cause) {
        ...
        } while (!prev$iv.addNext(closed, this_$iv));
    ...
    private final void helpClose(Closed<?> closed) {
        Object closedList = InlineList.m1743constructorimpl$default(null, 1, null);
        while (true) {
            LockFreeLinkedListNode prevNode = closed.getPrevNode();
            Receive previous = prevNode instanceof Receive ? (Receive) prevNode : null;
            if (previous == null) {
                break;
            } else if (!previous.mo1751remove()) {
                previous.helpRemove();
            } else {
                closedList = InlineList.m1748plusFjFbRPM(closedList, previous);
            }
        }
        if (closedList != null) {

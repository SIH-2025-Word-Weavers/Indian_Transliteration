    ...
    public void updateBackProgress(float progress, boolean leftSwipeEdge, int gravity) {
        ...
        if (width > 0.0f && height > 0.0f) {
            ...
            if (this.view instanceof ViewGroup) {
                int i = 0;
                for (ViewGroup viewGroup = (ViewGroup) this.view; i < viewGroup.getChildCount(); viewGroup = viewGroup) {
                    View childView = viewGroup.getChildAt(i);
                    childView.setPivotX(leftGravity ? (width - childView.getRight()) + childView.getWidth() : -childView.getLeft());
